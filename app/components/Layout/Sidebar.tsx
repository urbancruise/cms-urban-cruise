...
const filterMenuTree = (
  ...
  return items
    .map((item) => {
      // अगर item की खुद की permission है और user के पास नहीं है → skip
      if (item.perm && !hasPermission(item.perm)) {
        return null;
      }

      // Children को recursively filter करें
      if (item.children?.length) {
        const filteredChildren = filterMenuTree(item.children, hasPermission);

        // अगर parent के पास खुद की permission है → children हो या ना हो, दिखाओ
        if (item.perm) {
          return { ...item, children: filteredChildren };
        }

        // अगर parent के पास खुद की permission नहीं है और कोई visible child नहीं है → skip
      ...
      // Leaf item, permission pass कर चुका → दिखाओ
      return item;
    })
