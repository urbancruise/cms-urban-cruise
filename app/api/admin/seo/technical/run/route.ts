...
export async function POST(request: NextRequest) {
  try {
    ...
    const [[pages]] = (await pool.query(
      `SELECT COUNT(*) AS total, SUM(CASE WHEN canonical_url IS NULL OR canonical_url='' THEN 1 ELSE 0 END) AS missing_canonical FROM seo_pages`
    )) as any;
    const [[images]] = (await pool.query(
      `SELECT COUNT(*) AS total, SUM(CASE WHEN has_alt=0 THEN 1 ELSE 0 END) AS missing_alt FROM seo_images`
    )) as any;
    const [[links]] = (await pool.query(
      `SELECT COUNT(*) AS total, SUM(CASE WHEN is_broken=1 THEN 1 ELSE 0 END) AS broken FROM seo_internal_links`
    )) as any;
    const [[schemas]] = (await pool.query(
      `SELECT COUNT(*) AS total, SUM(CASE WHEN is_active=0 THEN 1 ELSE 0 END) AS inactive FROM seo_schemas`
    )) as any;
    ...
    checks.push([
      ...
      "/",
      "warning",
      "Consider optimizing images and enabling caching",
    ]);
    ...
    if (Number((pages as any).missing_canonical) > 0) {
      checks.push([
        ...
        "/",
        "warning",
        `${(pages as any).missing_canonical} pages missing canonical URL`,
      ]);
    } else {
    ...
    if (Number((links as any).broken) > 0) {
      checks.push([
        ...
        "/",
        "error",
        `${(links as any).broken} broken internal links detected`,
      ]);
    } else {
    ...
    if (Number((schemas as any).inactive) > 0) {
      checks.push([
        ...
        "/",
        "warning",
        `${(schemas as any).inactive} inactive schemas`,
      ]);
    } else {
