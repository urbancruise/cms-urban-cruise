...
export async function POST(request: NextRequest) {
  try {
    ...
    await pool.query(
      "DELETE FROM seo_core_web_vitals WHERE measured_at < DATE_SUB(NOW(), INTERVAL 1 DAY)"
    );

    const [pages] = (await pool.query(
      "SELECT page_path FROM seo_pages WHERE is_indexable = 1 LIMIT 20"
    )) as any;
