...
export async function GET(request: NextRequest) {
  try {
    ...
    const params: any[] = [];
    if (cityId) { where.push("city_id = ?"); params.push(Number(cityId)); }
    if (type) { where.push("keyword_type = ?"); params.push(type); }
    if (search) { where.push("keyword LIKE ?"); params.push(`%${search}%`); }

    const wc = where.join(" AND ");
    const [rows] = (await pool.query(`SELECT * FROM seo_keywords WHERE ${wc} ORDER BY updated_at DESC LIMIT 500`, params)) as any;
    const [countRows] = (await pool.query(`SELECT COUNT(*) AS total FROM seo_keywords WHERE ${wc}`, params)) as any;

    return NextResponse.json({ keywords: rows, total: Number((countRows as any[])[0]?.total) || 0 });
  } catch (err: any) {
  ...
  try {
    ...
    const [result] = await pool.query(
      `INSERT INTO seo_keywords (keyword, keyword_type, search_volume, difficulty, current_rank, target_rank, page_path, city_id, is_tracked)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [body.keyword, body.keyword_type || "secondary", body.search_volume || 0, body.difficulty || 0, body.current_rank, body.target_rank, body.page_path, body.city_id, body.is_tracked ? 1 : 0]
    );
