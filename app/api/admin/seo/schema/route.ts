...
export async function GET(request: NextRequest) {
  try {
    ...
    const [rows] = (await pool.query(`SELECT * FROM seo_schemas ORDER BY schema_type, page_path`)) as any;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [result] = await pool.query(
      ...
      [body.city_id || null, body.page_path || null, body.schema_type, JSON.stringify(body.schema_json), body.is_active ? 1 : 0]
    );
