...
function ensureParents(perms: string[]): string[] {
  ...
  const walk = (nodes: PermissionGroup[], ancestors: string[]) => {
    nodes.forEach((node) => {
      const currentAncestors = node.permKey ? [...ancestors, node.permKey] : ancestors;
...
export default function RolesPage() {
  ...
  const branchState = (node: PermissionGroup): "all" | "some" | "none" => {
    const keys = collectAllKeys([node]);
  ...
  const renderPermissionNode = (node: PermissionGroup, depth = 0): React.ReactNode => {
        ...
        >
          ...
          <span
            className={`text-sm flex-1 truncate ${
              state !== "none" ? "font-medium text-slate-900" : "text-slate-700"
            }`}
          >
            {node.label}
          </span>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setExpandedNodes(collectAllKeys(PERMISSION_TREE))}
                ...
                <p className="text-xs text-slate-500 mb-3">
                  Select which menu items users with this role can see. Click the arrow to
                  expand sub-sections. Parent checkboxes toggle all children. Selecting a
                  child will auto-include its parents.
                </p>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            <div className="p-6 text-center">
              ...
              <p className="text-xs text-slate-400 mb-4">
                This action cannot be undone. Users assigned to this role will lose their
                permissions.
              </p>
