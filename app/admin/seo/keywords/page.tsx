      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Track keyword rankings, volume, and difficulty ({data?.total || 0} total)
          </p>
        </div>
                ...
                {keywords.map((kw) => (
                  <tr key={kw.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`text-xs px-2 py-1 rounded-full font-medium border ${TYPE_COLORS[kw.keyword_type]}`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                          <div
                            className={`h-full ${kw.difficulty >= 70 ? "bg-red-500" : kw.difficulty >= 40 ? "bg-amber-500" : "bg-green-500"}`}
                    ...
                    <td className="px-6 py-4 text-sm">
                      {kw.current_rank ? (
                        <span
                          className={`font-bold ${kw.current_rank <= 3 ? "text-green-600" : kw.current_rank <= 10 ? "text-amber-600" : "text-slate-600"}`}
                        >
                          #{kw.current_rank}
                        </span>
                      ) : (
...
function KeywordModal({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit
        ? `/api/admin/seo/keywords/${keyword!.id}`
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                onChange={(e) => set("keyword_type", e.target.value)}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm"
              >
                <option value="primary">Primary</option>
                <option value="secondary">Secondary</option>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => set("search_volume", e.target.value)}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm"
              />
            </div>
            <div>
