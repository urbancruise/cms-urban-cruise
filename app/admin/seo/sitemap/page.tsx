    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            {pages.length} indexable pages in sitemap
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <button onClick={downloadSitemap} className="flex items-center gap-2 px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg shadow-sm">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 mb-6">
        ...
        <div className="flex items-center gap-2">
          <code className="flex-1 px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm font-mono text-slate-700">{sitemapUrl}</code>
          <button onClick={copyUrl} className="px-4 py-2 border border-slate-200 rounded-lg text-sm font-medium hover:bg-slate-50">
            {copied ? "✓ Copied" : "Copy"}
          </button>
          <a href="/sitemap.xml" target="_blank" rel="noopener noreferrer" className="p-2 border border-slate-200 rounded-lg hover:bg-slate-50">
            <MdOutlineOpenInNew className="w-4 h-4 text-slate-500" />
          </a>
        </div>
        <p className="text-xs text-slate-500 mt-3">
          Submit this URL to Google Search Console so search engines can discover your pages.
        </p>
      </div>

      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <h2 className="text-sm font-semibold text-slate-900 mb-4">Included Pages ({pages.length})</h2>
        {isLoading ? (
            ...
            {pages.map((p) => (
              <div key={p.id} className="flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:bg-slate-50">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-mono text-slate-800 truncate">{p.page_path}</p>
