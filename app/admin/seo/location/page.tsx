...
export default function SeoLocationPage() {
  ...
  const { data: pagesData, isLoading, mutate } = useSWR<{ pages: SeoPage[] }>(
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((p) => p.page_path.toLowerCase().includes(q) || (p.meta_title || "").toLowerCase().includes(q));
    return list;
    ...
    <div className="p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
        ...
        >
          ...
          <p className="text-xs text-slate-500 mt-1">{allPages.filter((p) => p.city_id).length} pages</p>
        </button>
        {cities.map((c) => {
          const stat = cityStats[c.id] || { total: 0, optimized: 0 };
          const pct = stat.total > 0 ? Math.round((stat.optimized / stat.total) * 100) : 0;
          return (
            <button
              key={c.id}
            ...
            >
              ...
              <div className="mt-2 h-1 bg-slate-100 rounded-full overflow-hidden">
                <div className={`h-full ${pct >= 80 ? "bg-green-500" : pct >= 50 ? "bg-amber-500" : "bg-red-500"}`} style={{ width: `${pct}%` }} />
              </div>
            </button>
          );
        })}
      ...
      <div className="relative mb-6 max-w-md">
        ...
        <input type="text" placeholder="Search by page path..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-slate-200 rounded-lg bg-white" />
      </div>
          ...
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Meta Title</th>
                <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Focus Keyword</th>
              ...
              {filtered.map((p) => {
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-3">
                      <p className="text-xs font-mono text-slate-800 truncate max-w-md">{p.page_path}</p>
                    </td>
                    <td className="px-6 py-3 text-sm text-slate-700 truncate max-w-xs">
                      {p.meta_title || <span className="text-red-500 italic">Missing</span>}
                    </td>
                    <td className="px-6 py-3 text-xs text-slate-600">
                      {p.focus_keyword || <span className="text-slate-400 italic">Not set</span>}
                    </td>
                    <td className="px-6 py-3 text-right">
                      <a href="/admin/seo/pages" className="inline-flex items-center gap-1 text-xs text-teal-600 hover:underline">
