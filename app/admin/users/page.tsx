...
import CityPermissionTree, { CityAccess } from "@/app/components/UI/CityPermissionTree";
import AvatarUpload from "@/app/components/UI/AvatarUpload";
import { WEBSITE_PERMISSION_TREE, collectAllKeys } from "@/lib/permissionTree";
    ...
    <form onSubmit={onSubmit} className="space-y-4">
            ...
            {roles.map((role) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="text-sm text-slate-900 flex-1">{role.name}</span>
      ...
      <div>
        <label className="flex items-center gap-2 text-sm font-medium text-slate-700 mb-2">
          ...
          City-wise Website Access ({formData.city_permissions.length} cities selected)
        </label>
        <p className="text-xs text-slate-500 mb-2">
          Grant this user access to specific Urban Cruise Website pages for each city.
          Only pages permitted by the selected role(s) are shown.
        </p>
    ...
    <div className="p-8">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage all users ({total} total)</p>
        </div>
        <button
          onClick={() => {
            resetForm();
              ...
              <tbody className="divide-y divide-slate-100">
                {users.map((user) => (
                  <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {user.city_permissions && user.city_permissions.length > 0 ? (
                        <div className="flex flex-wrap gap-1 max-w-xs">
                          {user.city_permissions.slice(0, 3).map((cp) => {
                            const city = user.cities?.find((c) => c.id === cp.city_id);
                            return (
                              <span
                                key={cp.city_id}
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div className="flex justify-between py-2 border-b border-slate-100">
                  ...
                  <span className="text-slate-900 font-medium">{selectedUser.email}</span>
                </div>
                ...
                <div className="py-2 border-b border-slate-100">
                  <span className="text-slate-500 block mb-2">
                    City-wise Website Access ({selectedUser.city_permissions?.length || 0}{" "}
                    cities)
                  </span>
